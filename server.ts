...
const DB_FILE = path.join(resolvedDirname, "database.json");
...
function readDB(): Database {
  ...
  try {
    ...
    console.error("Erro ao ler banco de dados JSON, recriando...", e);
    const initialDB = { clientes: [], tarefas: [], historico: [] };
    fs.writeFileSync(DB_FILE, JSON.stringify(initialDB, null, 2), "utf-8");
    return initialDB;
  }
}
